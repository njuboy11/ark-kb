...
async function extractPdfMinerU(filePath, config, opts) {
    ...
    while (Date.now() - startTime < timeoutMs) {
        ...
        if (state === "done") {
            ...
            if (totalPages)
                console.log(`[Ark KB] MinerU task ${taskId.slice(0, 8)} done (${totalPages} pages)`);
            break;
        }
        if (state === "failed") {
...
function isDocxComplex(filePath) {
    try {
        ...
        if (colsMatch && parseInt(colsMatch[1], 10) > 1)
            return true;
...
function isXlsxComplex(filePath) {
    try {
        ...
        if (/xl\/drawings\//.test(fileList))
            return true;
        if (/xl\/pivotTables\//.test(fileList))
            return true;
        if (/xl\/pivotCache\//.test(fileList))
            return true;
        // 6. Multiple sheets
        try {
        ...
        if (sheet1Xml) {
            if (sheet1Xml.includes("<conditionalFormatting"))
                return true;
            if (sheet1Xml.includes("<dataValidation"))
                return true;
...
function isPptxComplex(filePath) {
    try {
        ...
        if (mediaCount > 5)
            return true;
        // 3. SmartArt / diagrams
        if (/ppt\/diagrams\//.test(fileList))
            return true;
        // 4. OLE embedded objects
        if (/ppt\/embeddings\//.test(fileList))
            return true;
        // 9. Speaker notes (notesSlides with actual content)
        if (/ppt\/notesSlides\//.test(fileList))
            return true;
        // 9. Slide count > 20
        ...
        try {
            ...
            if (/<p:oleObj/.test(slide1Xml))
                return true;
            // 6. Embedded tables
            if (/<a:tbl[\s>]/.test(slide1Xml))
                return true;
            // 7. Animations/transitions
            if (/<p:anim/.test(slide1Xml))
                return true;
            // 8. Embedded video or audio
            if (/<p:video|audioFile/.test(slide1Xml))
                return true;
        }
        catch {
...
async function processDocx(filePath, embedder, chunkConfig, pdfConfig) {
...
// Xlsx processing
// ============================================================================
async function processXlsx(filePath, embedder, chunkConfig, pdfConfig) {
    const base = basename(filePath);
    const fileHash = await hashFile(filePath);
    // Auto-detect complexity from XML
...
// Pptx processing
// ============================================================================
async function processPptx(filePath, embedder, chunkConfig, pdfConfig) {
    const base = basename(filePath);
    const fileHash = await hashFile(filePath);
...
// HTML processing
// ============================================================================
async function processHtml(filePath, embedder, chunkConfig, pdfConfig) {
    const base = basename(filePath);
    const fileHash = await hashFile(filePath);
...
export class Ingester {
    ...
    _hashLocks = new Set(); // Race-condition guard: prevent concurrent ingest of same content
    ...
    async ingestFile(filePath) {
        ...
        const promise = (async () => {
            ...
            try {
                ...
                try {
                    switch (kind) {
                        ...
                        case "pdf":
                            ...
                            break;
                        case "docx":
                            entries = await processDocx(filePath, this.embedder, this.config.chunking, this.config.pdfParser);
                            break;
                        case "xlsx":
                            entries = await processXlsx(filePath, this.embedder, this.config.chunking, this.config.pdfParser);
                            break;
                        case "xls":
                            entries = await processXls(filePath, this.embedder, this.config.chunking, this.config.pdfParser);
                            break;
                        case "pptx":
                            entries = await processPptx(filePath, this.embedder, this.config.chunking, this.config.pdfParser);
                            break;
                        case "ppt":
                            entries = await processPpt(filePath, this.embedder, this.config.chunking, this.config.pdfParser);
                            break;
                        case "doc":
                            entries = await processDoc(filePath, this.embedder, this.config.chunking, this.config.pdfParser);
                            break;
                        case "html":
                            entries = await processHtml(filePath, this.embedder, this.config.chunking, this.config.pdfParser);
                            break;
                        default:
                            return { entries: 0, source: relative(this.knowledgePath, filePath), skipped: true };
                    }
                ...
                if (hashLock)
                    this._hashLocks.delete(hashLock);
            }
        })();
