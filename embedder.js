...
export class Embedder {
    ...
    parseResponse(data, api) {
        if (data.data && Array.isArray(data.data)) {
            const embeddings = data.data.sort((a, b) => (a.index ?? 0) - (b.index ?? 0)).map((item) => item.embedding);
        ...
        if (data.output?.embeddings && Array.isArray(data.output.embeddings)) {
            const embeddings = data.output.embeddings.sort((a, b) => (a.text_index ?? 0) - (b.text_index ?? 0)).map((item) => item.embedding);
