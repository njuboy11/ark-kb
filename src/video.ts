...
export async function extractKeyFrames(
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      proc.on("close", (code) => {
        if (code === 0) resolve();
        else reject(new Error(`frame extract exit ${code}: ${stderr.slice(-200)}`));
      });
...
export async function summarizeImage(
  imagePath: string,
  config: { apiKey: string; endpoint: string; timeoutMs: number },
): Promise<string> {
...
async function compressToProxy(inputPath: string, outputPath: string, resolution: number): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    proc.on("close", (code) => {
      if (code === 0) resolve();
      else reject(new Error(`compress exit ${code}: ${stderr.slice(-200)}`));
    });
    proc.on("error", reject);
  });
