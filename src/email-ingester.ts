...
export class EmailIngester {
  ...
  async init(): Promise<void> {
    ...
    this._initialized = true;  // Set immediately to prevent concurrent init
  ...
  async scan(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        let criteria: { since: Date } | { uid: string };
    ...
    if (textExts.includes(ext)) {
      ...
      try {
        ...
        const response = await this.askVLM(
          `当前可用知识库：${kbList}。请根据文档内容判断最适合放入哪些知识库。如果文档涉及多个领域，可以返回多个知识库。只回复 JSON: {"kbNames": ["知识库名1", "知识库名2"], "reason": "简短说明"}`,
          base64,
          mimeType,
        );
        const parsed = this._parseLLMJson(response);
